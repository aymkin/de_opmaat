# Session Notes

---

# 2025-12-03

## What We Did

### 1. Set Up the Learning Materials Directory

- Created `CLAUDE.md` to document this as a Dutch language learning directory
  for "De Opmaat" course (NT2, A2 level)

### 2. Converted PDF Test to Editable Markdown

- Converted `thema_4/toetsen_de_opmaat_t4.pdf` → `thema_4/toets_thema_4.md`
- The markdown file contains all 6 exercises from the original PDF

### 3. Practiced Opdracht 1 - Sentence Completion (Points 1-4 done) ✅

#### Point 1 - "want" structure

**Your answer:** Ik heb mijn huiswerk niet gemaakt want ik had geen tijd en mijn
computer was capot. **Correction needed:** capot → **kapot** **Status:**
Structure correct, small spelling fix needed

#### Point 2 - Starting with "Op het station"

**Your answer:** Op het station mis je de trein niet want het is een museum.
**Status:** Correct! (A joke about the train museum)

#### Point 3 - "Hoe laat" questions

**Your answers:**

1. Hoe laat gaat u vanavond sluiten? ✓
2. Hoe laat kom je bij mij thuis? ✓ **Status:** Both correct!

#### Point 4 - "als" clause

**Your answer:** Ze doet een stropdas aan als ze naar haar werk gaat.
**Status:** Correct! (You fixed stropdas spelling during session)

---

> 📘 **Grammar rules:** See `grammatica.md` for all grammar explanations in
> Russian.

---

# 2025-12-04

## What We Did

### 1. Practiced Opdracht 1 - Sentence Completion (Points 5-8 done) ✅

#### Point 5 - "omdat" clause at beginning

**Your answer:** Omdat mijn vrouw jaarig is, kopen we een cadeautje.
**Correction needed:** jaarig → **jarig** (één 'a') **Status:** Structure
correct, spelling fix needed

#### Point 6 - "waarom" question

**Your answer:** Waarom blijf je thius vanavond? **Correction needed:** thius →
**thuis** **Status:** Structure correct, spelling fix needed

#### Point 7 - "als" clause followed by main clause

**Your answer:** Als ik vrij ben, ik wandelen naar het park met mijn gezin.
**Corrections needed:**

- ik wandelen → **wandel ik** (inversie na bijzin!)
- wandelen → **wandel** (ik-vorm) **Status:** Grammar error - forgot inversion
  after subordinate clause

#### Point 8 - Starting with "Met de trein"

**Your answer:** Met de trein ga ik naar het werk elke dag. **Status:** ✅
Correct! Inversie is goed toegepast.

---

### 2. Practiced Opdracht 2 - Answer Questions in Complete Sentences ✅

#### Original Mistakes Made (before corrections):

| Vraag | Originele fout                                                | Type fout                     |
| ----- | ------------------------------------------------------------- | ----------------------------- |
| 1     | Vraag verkeerd begrepen ("Hoe gaat het?" i.p.v. "Hoe ga je?") | Begrijpend lezen              |
| 1     | envoudig → eenvoudig                                          | Spelling                      |
| 1     | progress → vooruitgang                                        | Engels → Nederlands           |
| 2     | mijn thuis → mijn huis                                        | Grammatica (thuis = bijwoord) |
| 2     | Nederlanse → Nederlandse                                      | Spelling                      |
| 2     | Woordvolgorde verkeerd                                        | Grammatica                    |
| 3     | and → en                                                      | Engels → Nederlands           |
| 3     | gegan → gegaan                                                | Spelling                      |
| 3     | hebben gegaan → zijn gegaan                                   | Hulpwerkwoord                 |
| 4     | weleen → weleens                                              | Spelling                      |
| 7     | geel vak → heel vaak                                          | Spelling/uitspraak            |
| 8     | dagkaar → dagkaart                                            | Spelling                      |

#### Final Answers (after corrections):

1. ✅ Ik ga met de fiets naar de Nederlandse les.
2. ✅ Van mijn huis naar de Nederlandse les ben ik ongeveer 20 minuten onderweg.
3. ✅ Mila en Simone zijn met de trein naar Maastricht gegaan.
4. ❌ Still has: weleen → **weleens**
5. ✅ Perfect!
6. ✅ Perfect!
7. ❌ Still has: vak → **vaak**
8. ✅ Perfect!

---

### Spelling Mistakes (2025-12-04)

- jarig (not jaarig - één 'a')
- thuis (not thius)
- weleens (not weleen)
- vaak (not vak)
- gegaan (not gegan)
- dagkaart (not dagkaar)
- eenvoudig (not envoudig)

### Grammar Mistakes (2025-12-04)

| Mistake            | Example                                                 | See grammatica.md |
| ------------------ | ------------------------------------------------------- | ----------------- |
| Inversie na bijzin | ❌ Als ik vrij ben, **ik wandel**... → ✅ **wandel ik** | Section 3         |
| ZIJN vs HEBBEN     | ❌ hebben gegaan → ✅ **zijn** gegaan                   | Section 4.3       |
| Thuis vs Huis      | ❌ mijn thuis → ✅ mijn **huis**                        | Section 5         |

---

### 3. Updated Grammar Reference File ✅

Rewrote `thema_4/grammatica.md` with comprehensive grammar rules in Russian:

- Want vs Omdat (word order differences)
- Als (verb to end)
- Inversie (including after subordinate clauses)
- Perfectum with zijn vs hebben
- Thuis vs Huis
- Question words with inversion
- Scheidbare werkwoorden (separable verbs)
- Quick reference cheat sheet

---

# 2025-12-06

## What We Did

### 4. Practiced Opdracht 3 - Perfectum ❌ (needs review)

**Score: 3/16** - Dit opdracht was moeilijk!

#### Regelmatige verba (0/8 correct)

| #   | Jouw antwoord   | Fout                    | Correct               |
| --- | --------------- | ----------------------- | --------------------- |
| 1   | heeft gestudert | spelling + uitgang      | heeft **gestudeerd**  |
| 2   | Heb geluisterd  | ✅ vorm ok              | Heb **geluisterd** ✅ |
| 3   | hebben wacht    | ge- ontbreekt           | hebben **gewacht**    |
| 4   | heb danst       | ge- ontbreekt           | heb **gedanst**       |
| 5   | heeft woont     | ge- ontbreekt + uitgang | heeft **gewoond**     |
| 6   | heb pakt        | ge- ontbreekt           | heb **gepakt**        |
| 7   | hebben luncht   | ge- ontbreekt           | hebben **geluncht**   |
| 8   | hebben winkeld  | ge- ontbreekt           | hebben **gewinkeld**  |

#### Onregelmatige verba (3/8 correct)

| #   | Jouw antwoord   | Fout                | Correct               |
| --- | --------------- | ------------------- | --------------------- |
| 1   | heb doen        | onregelmatig        | heb **gedaan**        |
| 2   | hebben gegeten  | ✅                  | ✅ Perfect!           |
| 3   | heb gekocht     | ✅                  | ✅ Perfect!           |
| 4   | hebben gezocht  | verkeerd werkwoord! | hebben **gezien**     |
| 5   | hebt genemen    | vervoeging + vorm   | **heeft** **genomen** |
| 6   | hebt geweenst   | hulpww + vorm       | **zijn** **geweest**  |
| 7   | hebben gekijken | onregelmatig        | hebben **gekeken**    |
| 8   | hebben gegeven  | ✅                  | ✅ Perfect!           |

---

### Grammar Mistakes (2025-12-06) - PERFECTUM

| #   | Mistake Type                  | Your Error                | Correct                             | See grammatica.md |
| --- | ----------------------------- | ------------------------- | ----------------------------------- | ----------------- |
| 1   | **ge- prefix vergeten** (6x!) | wacht, danst, pakt...     | **ge**wacht, **ge**danst...         | Section 4.1       |
| 2   | **'t kofschip** (-t/-d)       | gestudert, woont          | gestudeer**d**, gewoon**d**         | Section 4.1       |
| 3   | **Onregelmatige vormen**      | doen, gezocht, genemen... | ge**daan**, ge**zien**, ge**nomen** | Section 4.2       |
| 4   | **zien ≠ zoeken**             | gezocht (searched)        | ge**zien** (seen)                   | Section 4.2       |
| 5   | **Hulpwerkwoord zijn**        | hebt geweest              | **zijn** geweest                    | Section 4.3       |
| 6   | **Vervoeging: hebt/heeft**    | Sophia hebt               | Sophia **heeft**                    | Section 4.4       |

---

### Samenvatting Opdracht 3 - Te oefenen

| Fout type            | Frequentie | Prioriteit  |
| -------------------- | ---------- | ----------- |
| ge- prefix vergeten  | 6x         | ⭐⭐⭐ HOOG |
| Onregelmatige vormen | 5x         | ⭐⭐⭐ HOOG |
| 't kofschip (-t/-d)  | 2x         | ⭐⭐ MEDIUM |
| Hulpwerkwoord zijn   | 1x         | ⭐ LAAG     |
| Werkwoordvervoeging  | 1x         | ⭐ LAAG     |

---

### 5. Practiced Opdracht 4 - Tegenovergestelde (Opposites)

**Score: 5/8**

| #   | Woord    | Jouw antwoord | Status | Correct       |
| --- | -------- | ------------- | ------ | ------------- |
| 1   | leuk     | saai          | ✅     |               |
| 2   | snel     | langzaam      | ✅     |               |
| 3   | lelijk   | lief          | ❌     | **mooi**      |
| 4   | jong     | oud           | ✅     |               |
| 5   | dichtbij | verder        | ❌     | **ver/veraf** |
| 6   | vies     | schoon        | ✅     |               |
| 7   | laag     | hoog          | ✅     |               |
| 8   | duur     | goodkoop      | ❌     | **goedkoop**  |

### Vocabulary Mistakes (2025-12-06)

| Mistake           | Explanation                                                           |
| ----------------- | --------------------------------------------------------------------- |
| lelijk ↔ lief     | lief = милый/добрый, maar lelijk (некрасивый) ↔ **mooi** (красивый)   |
| dichtbij ↔ verder | verder = дальше (comparatief), maar dichtbij ↔ **ver/veraf** (далеко) |
| goodkoop          | Spelling: **goedkoop** (oe, niet oo)                                  |

---

### 6. Practiced Opdracht 5 - Schrijf een tekst ✅

**Task:** Write 50-70 words about your weekend using perfectum.

**Final text (~50 words):**

> Ik heb mijn ouders naar mijn huis uitgenodigd. We zijn samen naar Amsterdam
> gegaan. Mijn vader was blij om mijn zoon te zien. Mijn moeder heeft
> verrukkelijk eten gekookt. Ik heb mooie handschoenen voor mijn moeder gekocht.
> Mijn zoon heeft een speelgoedhuisje gebouwd. En mijn vrouw heeft opgeruimd. We
> hebben samen een leuke tijd gehad.

#### Original Mistakes:

| Fout            | Correct          | Type                        |
| --------------- | ---------------- | --------------------------- | ------ |
| mijn thuis      | mijn **huis**    | thuis = bijwoord            |
| naar bij        | naar / bij       | niet samen gebruiken        |
| heeft blij      | **was** blij     | blij zijn, niet hebben      |
| verrukulig      | verrukk**elijk** | spelling                    |
| eeten           | **eten**         | één 'e'                     |
| gekoken         | ge**kookt**      | regelmatig: koken → gekookt |
| schoonhanded    | **handschoenen** | spelling                    |
| mooi + zn       | **mooie** + zn   | bijvoeglijk naamwoord + -e  |
| gekoht          | ge**kocht**      | spelling                    |
| opruimte gedaan | **opgeruimd**    | scheidbaar: op              | ruimen |
| heeben          | **hebben**       | spelling                    |

#### Grammar Learned:

- Scheidbare werkwoorden in perfectum: **ge-** komt tussen voorvoegsel en stam
  - op|ruimen → op**ge**ruimd
  - uit|nodigen → uit**ge**nodigd

---

### 7. Practiced Opdracht 6 - Leesvaardigheid ✅

**Task:** Read text about NS Groepsticket Daluren and answer 4 questions.

**Initial Score: 2/4** → **Final Score: 4/4**

| Vraag | Onderwerp                      | Eerste antwoord | Correct         | Fout type                         |
| ----- | ------------------------------ | --------------- | --------------- | --------------------------------- |
| 1     | Maandag 8 uur, 6 personen      | B. nee ✅       | B. nee          | -                                 |
| 2     | Dinsdag heen+terug, 4 personen | B. €8 p.p. ❌   | C. €16 p.p.     | Enkele reis vergeten              |
| 3     | Zaterdag, 9 personen           | D. te groot ❌  | C. twee tickets | Niet aan meerdere tickets gedacht |
| 4     | Zondag, 3 personen             | B. nee ✅       | B. nee          | -                                 |

#### Key Learnings:

- **Enkele reis** = je hebt 2 tickets nodig voor heen én terug
- Als groep "te groot" is → meerdere tickets kopen is mogelijk
- "Ik + vrienden" = jij telt ook mee!

#### Spelling Mistakes:

- doordsweekse → **doordeweekse**
- group → **groep** (Nederlands, niet Engels)
- kein → **klein**

---

## Progress Summary

| Opdracht                                 | Status               |
| ---------------------------------------- | -------------------- |
| 1. Maak de zinnen compleet (1-8)         | ✅ Completed         |
| 2. Beantwoord de vragen                  | ✅ Completed         |
| 3. Perfectum (regelmatig + onregelmatig) | ✅ Completed (16/16) |
| 4. Tegenovergestelde                     | ✅ Completed (8/8)   |
| 5. Schrijf een tekst (50-70 woorden)     | ✅ Completed         |
| 6. Leesvaardigheid: Groepsticket         | ✅ Completed (4/4)   |

---

## All Spelling Corrections

- kapot (not capot)
- stropdas (not stroopdas)
- aan (not an)
- het station (not de station - het-woord!)
- mis (not miss - one 's')
- jarig (not jaarig - één 'a')
- thuis (not thius)
- weleens (not weleen)
- vaak (not vak)
- gegaan (not gegan)
- dagkaart (not dagkaar)
- eenvoudig (not envoudig)
- goedkoop (not goodkoop)
- verrukkelijk (not verrukulig)
- eten (not eeten)
- gekookt (not gekoken)
- handschoenen (not schoonhanded)
- gekocht (not gekoht)
- hebben (not heeben)
- doordeweekse (not doordsweekse)
- groep (not group - Nederlands, niet Engels)
- klein (not kein)

## Vocabulary to Remember

- lelijk ↔ **mooi** (not lief)
- dichtbij ↔ **ver/veraf** (not verder)

## Grammar to Remember

- **Scheidbare werkwoorden in perfectum:** ge- tussen voorvoegsel en stam
  - op|ruimen → op**ge**ruimd
  - uit|nodigen → uit**ge**nodigd
- **Bijvoeglijk naamwoord + zelfstandig naamwoord:** meestal + **-e**
  - mooi → mooie handschoenen
